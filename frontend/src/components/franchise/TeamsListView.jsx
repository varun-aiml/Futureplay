...
const TeamsListView = () => {
  ...
  // Fetch tournaments and bookings
  useEffect(() => {
    const fetchTournamentsAndBookings = async () => {
      try {
        setIsLoading(true);
        // Fetch only the organizer's tournaments instead of all tournaments
        const tournamentsResponse = await getOrganizerTournaments();
        const tournamentsData = tournamentsResponse.data.data; // Extract the data array from the response
        ...
        for (const tournament of tournamentsData) {
            try {
                const response = await getTournamentBookings(tournament._id);
                const tournamentBookings = response.data.data;
                // Add tournament name to each booking for easier reference
                const bookingsWithTournamentName = tournamentBookings.map(booking => ({
                  ...booking,
                  tournamentName: tournament.name
                }));
                allBookings.push(...bookingsWithTournamentName);
              } catch (bookingError) {
                // Handle the case where user doesn't have access to this tournament's bookings
                console.warn(`Cannot access bookings for tournament ${tournament.name}: ${bookingError.message}`);
                // Continue with the next tournament instead of breaking the entire loop
              }
        }
  ...
  // Get event name from tournament and event IDs
  const getEventName = (tournamentId, eventId) => {
    const tournament = tournaments.find(t => t._id === tournamentId);
    if (!tournament || !tournament.events) return 'Unknown Event';
  ...
  return (
    <div className="mb-6">
      ...
      <div className="mb-4">
        ...
        <select
          id="tournamentFilter"
          className="bg-gray-700 border border-gray-600 text-white rounded-md px-3 py-2 w-full max-w-md focus:outline-none focus:ring-2 focus:ring-red-500"
